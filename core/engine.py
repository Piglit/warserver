...
class Game:
	...
	def __init__(self, settings):
		...
		for col in range(0, 8):
			...
			for row in range(0, 8):
				sector = {
					...
					"Difficulty_mod":	0,
					# self.settings["Game Difficulty Level"] is added to determine the difficulty level. GM may change this value for each sector
					"??":				0,	#TODO
					"Hidden":			False,
					"Name":				"Sector",
					...
					"always_enterable":	False,
					#if true, you can always enter the sector, regardless of enemies or neighbouring sectors or ships inside. However Hidden and blocked still prevent entering.
					"last_update":		0.0,
				}
		...
		for _ in range(self.settings["empty sectors"]):
	...
	def get_map(self, client=None):
		...
		with self._lock:
			...
			if (("ALL") in self.events_get_map):
				changes += self.events_get_map.get(("ALL"))
